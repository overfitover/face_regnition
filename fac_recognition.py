...
def main():
    ...
    with tf.name_scope('final_training_ops'):
        #fc1
        weights1 = tf.Variable(tf.truncated_normal([BOTTLENECK_TENSOR_SIZE, middle1], stddev=0.001))
        biases1 = tf.Variable(tf.zeros([middle1]))
        layer1 = tf.nn.relu(tf.matmul(bottleneck_input, weights1) + biases1)
        #fc2
        weights2 = tf.Variable(tf.truncated_normal([middle1, middle2], stddev=0.001))
        biases2 = tf.Variable(tf.zeros([middle2]))
        logits = tf.matmul(layer1, weights2)+biases2

        # fc3
        weights3 = tf.Variable(tf.truncated_normal([middle2, n_classes], stddev=0.001))
        biases3 = tf.Variable(tf.zeros([n_classes]))
        logits = tf.matmul(logits, weights3) + biases3
    ...
    with tf.Session() as sess:
        ...
        # 训练过程
        saver.save(sess,"incepton_model_save/inception_dec_2015.ckpt")
